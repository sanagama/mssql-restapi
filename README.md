...
You can optionally configure the SQL Server instance to connect to via environment variables. The prototype connects to SQL, dynamically discovers instance data and metadata and provides a RESTful interface to browse the Server, Databases, Tables and Columns. For fun, it also has a REST end-point to generate CREATE scripts for Databases and Tables. The prototype is built using .NET Core 2.0 and can run on Linux, macOS, Windows and in Docker.
